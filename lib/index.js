 ...
 */
module.exports.extend = function extend (parent, specs) {
  ...
  specs.forEach(function (spec) {
    var getMessage = util.getName(spec.message) === 'String'
      ? compileStringTemplate(spec.message)
      : spec.message
    ...
    if (spec.errors) {
      spec.errors.forEach(function (errorSpec) { extend(CustomError, errorSpec) })
    }
  })
